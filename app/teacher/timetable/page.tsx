...
export default function TeacherTimetablePage() {
  ...
  const fetchTimetable = async () => {
    ...
    try {
      ...
      // Fetch timetable where teacher_name matches
      const { data: timetableData } = await supabase
        .from('timetables')
        .select('*')
        .or(`teacher_name.ilike.%${teacher.full_name}%,teacher_name.ilike.%${email}%`)
        .eq('is_active', true)
        .order('day_of_week')
        .order('period_number')
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex justify-between items-center mb-6 print:hidden">
          <div>
            ...
            <p className="text-gray-600 text-sm">First Term 2024/2025</p>
          </div>
                        ...
                        {entry ? (
                          <div className="bg-blue-50 border border-blue-200 rounded p-2 text-xs">
                            ...
                            <p className="text-blue-700 flex items-center gap-1 mt-1">
                              <MapPin size={10}/> {entry.class_id}
                            </p>
                            {entry.room_number && (
                              <p className="text-blue-600 flex items-center gap-1">
                                <User size={10}/> {entry.room_number}
                              </p>
                            )}
                          </div>
                        ) : (
